<nav x-data="{ open: false }"
        ...
        <div class="flex justify-between h-16 items-center">
            ...
            <div class="flex items-center space-x-8">
                ...
                <div class="hidden sm:flex space-x-6">
                    ...
                    {{-- Panel Penulis Dropdown --}}
                    @if(auth()->check() && auth()->user()->isPenulis())
                        <x-dropdown align="bottom" width="48">
                            <x-slot name="trigger">
                                <button
                                    class="flex items-center space-x-1 text-white font-medium hover:text-green-200 focus:outline-none">
                                    <span>{{ __('Panel Penulis') }}</span>
                            ...
                            <x-slot name="content">
                                <div class="rounded-xl bg-white dark:bg-green-700 shadow-lg overflow-hidden">
                                    <x-dropdown-link :href="route('penulis.books.index')">
                                        {{ __('Buku Saya') }}
                                    </x-dropdown-link>
                                    <x-dropdown-link :href="route('penulis.books.create')">
                                        {{ __('Tambah Buku Baru') }}
                                    </x-dropdown-link>
                                    <x-dropdown-link :href="route('penulis.chapters.create', 1) {{-- nanti ganti book_id dinamis --}}">
                                        {{ __('Tambah Chapter') }}
                                    </x-dropdown-link>
                                </div>
                            </x-slot>
                    ...
                    <x-slot name="content">
                        <div class="rounded-xl bg-white dark:bg-green-700 shadow-lg overflow-hidden">
                            ...
                            <x-dropdown-link :href="route('profile.edit')"
                                class="text-gray-700 dark:text-white hover:bg-green-300 dark:hover:bg-green-500 rounded-xl">
                                {{ __('Profile') }}
                            </x-dropdown-link>
                            <form method="POST" action="{{ route('logout') }}">
                                @csrf
                                <x-dropdown-link :href="route('logout')"
                                    class="text-gray-700 dark:text-white hover:bg-green-300 dark:hover:bg-green-600 rounded-xl"
            ...
            <div class="-me-2 flex items-center sm:hidden">
                <button @click="open = ! open"
                    class="inline-flex items-center justify-center p-2 rounded-md text-white hover:bg-green-600 focus:outline-none focus:bg-green-600 transition duration-150 ease-in-out">
    ...
    <div :class="{'block': open, 'hidden': ! open}" class="hidden sm:hidden bg-green-600 dark:bg-green-700">
        <div class="pt-2 pb-3 space-y-1">
            ...
            {{-- Panel Penulis Dropdown Mobile --}}
            @if(auth()->check() && auth()->user()->isPenulis())
                <x-responsive-nav-link :href="route('penulis.books.index')">
                    {{ __('Buku Saya') }}
                </x-responsive-nav-link>
                <x-responsive-nav-link :href="route('penulis.books.create')">
                    {{ __('Tambah Buku Baru') }}
                </x-responsive-nav-link>
                {{-- Chapter biasanya pilih buku dulu, nanti bisa buat page khusus --}}
                <x-responsive-nav-link :href="route('penulis.chapters.create', 1)">
                    {{ __('Tambah Chapter') }}
                </x-responsive-nav-link>
            @endif
        </div>
